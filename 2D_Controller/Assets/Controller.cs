...
public class Controller : MonoBehaviour {
    ...
    void Update()
    {
        ...
        ////////////////////////////////////////////////////////////////////////////////
        if (Input.GetTouch(0).phase == TouchPhase.Began)        // 화면 터치상태라면.
        {
            PNT_A.transform.position = Input.GetTouch(0).position;     // 처음 터치시 포인트의 위치로 지정함.
            isTouch = true;                                     // 터치를 활성화.
        }

        if (Input.GetTouch(0).phase == TouchPhase.Ended)        // 터치가 아니라면
        {
            isTouch = false;                                    // 터치활성화를 끔.
        }
        if (isTouch)                                            // 터치활성화 상태라면
        {
            PNT_B.transform.position = Input.GetTouch(0).position;     // 드래그 포인트의 위치를 지속적으로 터치 포인트의 위치로 갱신시킴.
    ...
    void trigonometric_function()
    {
        ...
        //if ((Angle < tempAngle.y && Angle > 1) || (Angle < tempAngle.y + 180))
        {
